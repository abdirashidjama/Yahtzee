...
public class RerollTestStepDefinitions extends AbstractGameStepDefinition{
	...
	@Given("player recieves dice and {int}, {int}, {int}, {int} a couple")
	public void player_recieves_dice_and_a_couple(Integer int1, Integer int2, Integer int3, Integer int4) {
	    // Write code here that turns the phrase above into concrete actions
		
		game.getPlayers()[0].rollDice();
		for(int i=0; i<5;i++) {
			if(i!=int1-1 || i!=int2-1 || i!=int3-1 || i!=int4-1) {
				game.getDice()[i].setValue(0);
			}
		}
	}

	@When("player rolls the other dice that are not among the {int}, {int}, {int}, {int}")
	public void player_rolls_the_other_dice_that_are_not_among_the(Integer int1, Integer int2, Integer int3, Integer int4) {
	    // Write code here that turns the phrase above into concrete actions
